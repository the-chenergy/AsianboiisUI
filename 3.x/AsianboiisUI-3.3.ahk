...
global DefaultMagicScrolling, IsMacKeyboard, AlwaysMaxPerformance
...
ResetModifiers(mouseButtons := true)
{
	...
	SendInput, {Esc} ; to prevent anything from popping up
}
			...
			if (!IsFnKeyComboUsed) ; only toggle when there are no prior fn-hotkeys used
			{
				if (IsImeOn := !IsImeOn)
				{
					...
					else
					{
						...
						MsgBox, % "The Shuangpin (双拼) IME dictionary could not be found. Please contact Asianboii for the dictionary file!!`n`nFile path: ""./Data/Ime.txt"""
					}
				}
				else
				{
...
*LButton:: SendInput, {RButton Down}
