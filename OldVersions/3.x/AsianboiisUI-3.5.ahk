...
global IsBackSpaceFnDown := false
...
global BackSpaceKeyName := (IsMacKeyboard ? "RWin" : "RAlt")
...
ResetModifiers(mouseButtons := true)
{
	...
	IsFnLockOn := IsFnDown := IsFnBackSpaceDown := false
...
{
	if (IsBackSpaceFnDown)
		return ; ignore if the thumb-baskspace key (used as the other fn) is already held down
...
; used as main logic of fn key-up, for either fn or thumb-bs key
InternalFnUp()
{
...
;{ mac/pc keyboard layout definitions
...
BackSpaceDown()
{
	...
	IsFnDown := IsBackSpaceFnDown := true
...
BackSpaceUp()
{
	...
	IsFnLockOn := IsFnDown := IsBackSpaceFnDown := false
	
	if (A_PriorKey = BackSpaceKeyName && !IsFnKeyComboUsed && !IsSpaceComboUsed)
		PressBackSpace()
