...
t::
	if (isCapsDown) {
		...
		SendInput, % GetKeyState("CapsLock", "T") ? "Y" : "y"
	}
return
...
d::
	if (isCapsDown) {
		...
		SendInput, % GetKeyState("CapsLock", "T") ? "E" : "e"
	}
return
...
+Capslock Up::
	...
	if (A_PriorKey = "CapsLock" && !isScrolled) {
...
Tab Up::
	...
	if (isInQwerty)
		Suspend, On
	else if (A_PriorKey = "Tab" && !GetKeyState("Space", "P") && !isScrolled)
		Send, {Blind}{Tab Down}
...
+WheelDown::
	...
	if (GetKeyState("Space", "P") || GetKeyState("RButton", "P")) {
		if (_isScrollingDown) {
			Send, {WheelRight}
			Send, {WheelRight}
		} else { ; scrolling up
	...
	if (isNormalScrolling ^ GetKeyState("Tab", "P")) {
		if (_isScrollingDown)
			Send, {WheelDown}
		else
			Send, {WheelUp}
